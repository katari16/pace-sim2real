...
parser.add_argument("--max_frequency", type=float, default=5.0, help="Maximum frequency for the chirp signal in Hz.")
...
def main():
    ...
    friction = torch.tensor([0.01] * len(joint_ids), device=env.unwrapped.device).unsqueeze(0)
    ...
    # just print joint limits
    low_limits = articulation.data.soft_joint_pos_limits[0, joint_ids, 0]
    high_limits = articulation.data.soft_joint_pos_limits[0, joint_ids, 1]


    print("\n" + "="*60)
    print(f"{'Joint Name':<25} | {'Lower Limit':<12} | {'Upper Limit':<12}")
    print("-" * 60)
    for i, name in enumerate(joint_order):
        print(f"{name:<25} | {low_limits[i]:>12.4f} | {high_limits[i]:>12.4f}")
    print("="*60 + "\n")
    # end print joint limits
    env.reset()
    ...
    default_positions = articulation.data.default_joint_pos[0, joint_ids]
    test_action = trajectory[4 % num_steps, :]
    print("\n" + "="*50)
    print(f"{'Joint Name':<25} | {'Default Pos (rad)':<20}")
    print("-" * 50)
    
    # Zip the names with the values to print them side-by-side
    for name, pos in zip(joint_order, test_action - default_positions):
        print(f"{name:<25} | {pos.item():>12.4f}")
    print("="*50 + "\n")
...
# -----
    ...
    # Zip the names with the values to print them side-by-side
    for name, pos in zip(joint_order, test_action):
        print(f"{name:<25} | {pos.item():>12.4f}")
    print("="*50 + "\n")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # ISSUE: on the robot we are sending abs targets and the recorded data is abs.
            # Isaac applies actions as offsets to default positions. subtract the default position before calling action bcs afterwards it will add them. otherwise they will be added twice 
            # compute actions
            abs_target = trajectory[counter % num_steps, :]
            dof_pos_buffer[counter, :] = env.unwrapped.scene.articulations["robot"].data.joint_pos[0, joint_ids] - bias[0]
            actions = torch.zeros(env.action_space.shape, device=env.unwrapped.device)
            actions = (abs_target).unsqueeze(0).repeat(env.unwrapped.num_envs, 1)
            # apply actions
    ...
    # Create a 4x3 grid (4 legs, 3 joints per leg)
    ...
    for i in range(len(joint_ids)):
        ...
        # Plot actual position vs target
        ax.plot(t.cpu().numpy(), dof_pos_buffer[:, i].cpu().numpy(), label="Sim Actual", color='tab:blue')
        ax.plot(t.cpu().numpy(), dof_target_pos_buffer[:, i].cpu().numpy(), label="Sim Target", linestyle='--', color='tab:orange')
        
        # Formatting
        ax.set_title(f"{joint_order[i]}", fontsize=10)
        ax.grid(True, which='both', linestyle='--', alpha=0.5)
        
        # Only label Y on the first column (Hips, indices 0, 3, 6, 9)
        if i % 3 == 0: 
            ax.set_ylabel("Position [rad]")
        
        # Only label X on the bottom row (indices 9, 10, 11)
        if i >= 9: 
    ...
    plt.suptitle(f"Chirp Trajectory Comparison: {args_cli.min_frequency}Hz to {args_cli.max_frequency}Hz", fontsize=16)
